...
#     global sheet  # Assuming you have a global worksheet variable

#     net_profit = None
#     total_sales = None
    
#     for row in sheet.iter_rows():
#         if row[0].startswith == "NET PROFIT":
#             net_profit = row[1].value
#         elif row[0].value == "Sales":
#             total_sales = row[1].value
        
#         if net_profit is not None and total_sales is not None:
...
def insert_bold_and_expenditure_into_excel(pdf_path, excel_path, page_numbers):
    global row_index
    row_index += 1
    target_words = [
        "Cost of sales",
    ...
    bold_expenditure_data = []
    current_bold_word = None
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            for line in lines:
                ...
                if any(word.lower() in line_lower for word in target_words):
                    current_bold_word = line.strip()
                elif current_bold_word and "page" not in line_lower:
                    expenditure = line.strip()
                    # Exclude unnecessary lines
                    if expenditure and not any(keyword in line_lower for keyword in ["£ £ £ £", "for the year ended", "profit and loss account", "carried forward", "brought forward"]):
                        # Remove numeric figures from expenditure using regex
                        expenditure = re.sub(r'[\d,]+', '', expenditure)
                        if expenditure.strip() and len(expenditure.strip()) > 1:
                            bold_expenditure_data.append((current_bold_word, expenditure.strip()))

    # Open the existing Excel sheet
    workbook = openpyxl.load_workbook(excel_path)
    sheet = workbook.active
    
    # Find the next empty row and leave two rows below
    next_empty_row = sheet.max_row + 3

    # Set font size to 9
    font = openpyxl.styles.Font(size=9)
    
    # Insert the bold word and corresponding expenditure data into the Excel sheet
    for bold_word, expenditure in bold_expenditure_data:
        sheet.cell(row=next_empty_row, column=1, value=bold_word).font = font
        sheet.cell(row=next_empty_row, column=2, value=expenditure).font = font
        next_empty_row += 1
...
def extract_numeric_value(value):
    # Remove brackets and underlines
    cleaned_value = re.sub(r'[()\[_\]]', '', value)
    
    # Remove non-numeric characters except for the decimal point
    numeric_part = re.sub(r'[^\d.]', '', cleaned_value)
        ...
        for i, page_number in enumerate(page_numbers, start=1):
            ...
            for line in profit_loss_text.split("\n"):
                ...
                if match:
                    ...
                    # Assuming this_year and last_year are your extracted values
                    this_year_float = extract_numeric_value(this_year)
                    last_year_float = extract_numeric_value(last_year)
    ...
    if pdf_path:
        ...
        if workbook:
            ...
            if excel_path:
                ...
                #calculate_net_profit_margin(excel_path)
                workbook.save(excel_path)
                insert_bold_and_expenditure_into_excel(pdf_path, excel_path, page_numbers)
                output = "output"
